...
\begin{thm}[Hitchin]
  ...
  \end{equation*}
  where $\BB$ is a complex vector space of complex dimension $k$, such that its components Poisson commute and generic fibres of it are open sets in some submanifolds which are isomorphic to complex tori $\CC^k/\ZZ^{2k}$.
\end{thm}
...
\begin{equation*}
  \AA^c=\AA_{\delbar}\times \Omega^{1,0}(X,\End E),
\end{equation*}
which is a complex affine space over $\Omega^{0,1}(X,\End E) \oplus \Omega^{1,0}(X,\End E)$ and $\AA^c=T^*\AA_{\delbar}$. This space carries a natural symplectic form 
\begin{equation*}
...
\begin{align*}
  \mu :\AA^c=\AA_{\delbar}\times \Omega^{1,0}(X,\End E)&\longrightarrow (\Lie\GG^c)^*=\Omega^2(X,\End E) \\
  ...
  \end{equation*}
  Therefore we get a symplectic quotient $$T^*\Bun(n,d)=\AA^{c,s}/\GG^c=\mu^{-1}(0)/\GG^c,$$ where $\AA^{c,s}=\AA_{\delbar}^s\times \Omega^{1,0}(X,\End\ E)$ denotes the open subset corresponding to stable holomorphic vector bundles.
  Note also that for a pair $(\delbar_{\ve{E}},\varphi)\in \AA^c$ we have that $\mu(\delbar_{\ve{E}},\varphi)=0$ if $\delbar_{\ve{E}} \varphi=0$, that is, if $\varphi\in H_{\delbar_{\ve{E}}}^{1,0}(X, \End\ E)=H^0(X,\End\ \ve{E} \otimes K_X)$, just as we wanted.
