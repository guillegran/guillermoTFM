...
J(X):=	 \frac{H^1(X,\OO_X)}{H^1(X,\ZZ)} \cong \frac{\CC^g}{\ZZ^{2g}}.
	 ...
	 \begin{align*}
	   ...
	   \end{equation*}
	   Therefore we can identify the quotient set $\AA_{\delbar}/\GG^c$ with the set of isomorphism classes of holomorphic vector bundles of rank $\rk E$ and degree $\deg E$. Even though $\AA_{\delbar}$ and $\GG^c$ are infinite-dimensional spaces, using analytic techniques [REF:Kobayashi] they can be given the structure of \emph{Banach manifolds} (roughly, spaces locally modeled by Banach spaces) and this quotient space can be precisely constructed. This has however a serious problem: the obtained space is not Hausdorff. However, if we restrict ourselves to the class of \emph{stable vector bundles}, with the same analytic methods we can obtain a ``good'' moduli space.
	   \begin{defn}
	     Let $\ve{E}$ be a holomorphic vector bundle over a compact Riemann surface $X$. We define the \emph{slope} of $\ve{E}$ as the number
	     \begin{equation*}
	       \mu(\ve{E})=\deg \ve{E}/ \rk \ve{E}.
	     \end{equation*}
	     We say that the vector bundle $\ve{E}$ is \emph{stable} if for every holomorphic subbundle $\ve{F}\subset \ve{E}$, $$\mu(\ve{F})<\mu(\ve{E}).$$ 
	     Fixed $E$ a complex vector bundle of rank $n$ and degree $d$, we consider $\AA_{\delbar}^s\subset \AA_{\delbar}$ the subset of stable holomorphic bundles $(\ve{E},\delbar_{\ve{E}})$ and define the \emph{moduli space of stable holomorphic vector bundles} of rank $n$ and degree $d$ as the quotient
	     \begin{equation*}
...
In this section we are going to give a gauge-theoretical approach to holomorphic vector bundles. This will allow us to give another analytical construction of the moduli space, this time as the space of solutions (up to gauge equivalence) to some differential equation.
