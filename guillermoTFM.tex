...
p^*L	  \arrow{rr}\arrow{dd} && L\arrow{dd}[anchor=west]{p} \\ 
	...
	\begin{align*}
	  ...
	  \begin{proof}
		...
		d_A \mu(B) =\left.\frac{d}{dt} \right|_{t=0} \mu(A+tB)=\tfrac{i}{2}\left.\frac{d}{dt} \right|_{t=0}    [A+tB,A^{\dagger}+tB^\dagger]=\tfrac{i}{2} \left([B,A^{\dagger}] + [A,B^{\dagger}]\right).
	      ...
	      Now
	      \begin{equation*}
		\esc{d_A\mu(B),H}=\tfrac{i}{2}\tr([B,A^\dagger]H^\dagger+[A,B^{\dagger}]H^\dagger)=-\tfrac{i}{2} \tr([B,A^\dagger]H+[A,B^{\dagger}]H)
	      \end{equation*}
	      Using the cyclic property of the trace one gets
	      \begin{equation*}
		\esc{d_A\mu(B),H}=-\tfrac{i}{2}\tr(B[A^\dagger,H]-[H,A]B^\dagger)=-\mathrm{Im}\ \tr((B [A,H])).
	      \end{equation*}
	      On the other hand
	      \begin{equation*}
		\omega(B,\vec{H}(A))=-\mathrm{Im}\ \tr(B[A,H])=\esc{d_A\mu(B),H}.
	      \end{equation*}
	      This finishes the proof of the lemma.
	    \end{proof}
	    In the general case, the symplectic structure locally has the form
	    \begin{equation*}
	    \omega(\varphi_1,\varphi_2)=2i \int_X \mathrm{Im}\  \tr(\varphi_1\varphi_2)=2i \int_X \mathrm{Im}\ [\tr(\phi_1\phi_2^\dagger)] \otimes \alpha_1 \wedge \bar{\alpha_2}.
	    \end{equation*}
  ...
  \end{equation*}
  which has Krull dimension 1, since $\dim \OO_{X,x}=1$ and $\dim \CC[\varphi(x)]=0$. Therefore, $S_\varphi $ is a $1$-dimensional scheme.
...
The sum of these imbeddings gives a homomorphism $\LL^{-n}\rightarrow \Sym(\LL^{-1})$ that locally looks like the characteristic polynomial,
    \begin{align*}
      ...
      \end{align*}
      The image of this homomorphism generates an ideal sheaf $\II$ and the spectral curve can be defined as $\RSpec(\Sym(\LL^{-1})/\II)$. The Cayley-Hamilton theorem assures that $\II \subset \ker \tilde{\varphi}$. When the spectral curve is integral, the minimal and characteristic polynomial generate the same ideal and so the spectral curve coincides with the scheme $S_\varphi$.  

      The spectral correspondence is now clear, the restriction $\pi=p|_{S_\varphi}:S_\varphi \rightarrow X$ is an finite morphism of degree $n$, so in particular it is affine and $\pi_*$ gives an equivalence of categories
  \begin{center}
    \begin{tikzcd}
    ...
    \end{tikzcd}
  \end{center}
  Of course $\pi_*\OO_{S_\varphi}=\Sym(\LL^{-1})/\ker \tilde{\varphi}$, so $\varphi$ endows $E$ with a $\pi_*\OO_{S_\varphi}$-module structure, that must correspond to a quasi-coherent sheaf $\MM$ of $\OO_{S_\varphi}$-modules such that $\pi_* \MM=\EE$. If $S_\varphi$ is smooth then the sheaf $\MM$ is locally free. Moreover, since $\deg(\pi)=n$, $\pi_*\OO_{S_\varphi}$ has the same rank that $\EE$, $n$, as an $\OO_X$ module, so $\MM$ must have rank $1$. Conversely, from a line bundle $M$ on $S_\varphi$ we recover $\pi_*M$ a rank $n$ vector bundle endowed with a $\pi_*\OO_{S_\varphi}$-module structure. 
...
To understand this conditions let us consider the norm map
  \begin{align*}
    ...
    \end{defn}
    Summing up, we have that the fiber $H^{-1}(b)$ of the Hitchin map corresponds to the Prym variety $P(S_b,X)$, wich can be shown to be a complex torus of dimension equal to $\dim(\BB_{\SL(n,\CC)})$ and giving again an integrable system.
