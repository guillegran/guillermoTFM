...
For the purpose of this work it is necessary to work on an infinite-dimensional context, so we can construct the moduli spaces as quotients of infinite-dimensional manifolds. To do this, we need to consider an infinite-dimensional analogous to differential calculus and the notion of infinite-dimensional smooth manifold. Here we just state the basic definitions and results, which are a direct generalization of the classical ones, and refer to \cite{abrahammarsdenratiu,lang} for more details.
\begin{defn}
  Let $X$ and $Y$ be Banach spaces and $U\subset X$ an open set. Let $f:X\rightarrow Y$ be a continuous map. We say that $f$ is \emph{differentiable} at a point $x_0 \in U$ if there exists a continuous linear map $d_{x_0}f:X\rightarrow Y$ such that 
  \begin{equation*}
    \lim_{h\rightarrow 0} \frac{\norm{f(x_0+h)-f(x_0)-d_{x_0}f(h)}_Y}{\norm{h}_X}=0.
  \end{equation*}
  ...
  \begin{align*}
    df :U&\longrightarrow L(X,Y)\\ 
      x &\longmapsto d_xf, 
    \end{align*}
    where $L(X,Y)$ denotes the set of bounded linear maps $X\rightarrow Y$. If $df$ is continuous, we say that $f$ is \emph{of class} $C^1$. Inductively, we define maps of class $C^p$, for $p\in \mathbf{N} \cup \left\{ \infty \right\}$. If $f$ is of class $C^{\infty}$, we say that it is \emph{smooth}. If $f$ is bijective map of class $C^p$ such that its inverse is also of class $C^p$, we say that $f$ is a $C^p$-\emph{diffeomorphism}. If we do not specify, by a \emph{diffeomorphism} we just mean a $C^\infty$-diffeomorphism.
\end{defn}
...
\begin{defn}
  ...
  \begin{enumerate}
    ...
    \item Each $\varphi_\alpha:U_\alpha \rightarrow V_\alpha$ is a homeomorphism of $U_\alpha$ onto some open subset of a Banach space $V_\alpha \subset Y_\alpha$ and for any $\alpha, \beta$, $\varphi_\alpha(U_\alpha \cap U_\beta)$ is open in $Y_\alpha$.
    \item The map 
      \begin{equation*}
...
Let $X$ be a Banach smooth manifold and $x\in X$. Consider triples $(U,\varphi,v)$, where $(U,\varphi)$ is a chart at $x$ with $\varphi:U\rightarrow Y$, $Y$ a Banach space, and $v\in Y$. We say that two such triples $(U,\varphi,v)$ and $(U',\varphi',v')$ are \emph{equivalent} if 
\begin{equation*}
...
\begin{align*}
  ...
  \section{Symplectic manifolds and the moment map}
  \begin{defn}
    ...
    \begin{enumerate}
      ...
      \item For each $x\in X$, $\omega_x$ is non-degenerate, i.e., if $\omega_x(u,v)=0$ for all $v\in T_xX$, then $u=0$M
...
\end{equation*}
where $i_v \omega$ denotes the contraction. Since $\omega$ is closed, $v$ is symplectic if and only if the $1$-form $i_v \omega$ is closed. We say that $v$ is \emph{Hamiltonian} if $i_v \omega$ is exact. In that case, there exists a function $f:X\rightarrow \RR$, called the \emph{Hamiltonian} of $v$ such that
\begin{equation*}
  df=-i_v \omega.
\end{equation*}
The minus sign in the last equality is just a widely used convention. Of course, if the first de Rham cohomology of $X$ vanishes, $H^1_{\mathrm{dR}}(X)=0$, then every symplectic vector field is Hamiltonian. In particular, every symplectic vector field is \emph{locally Hamiltonian}.
...
\begin{equation*}
  \rho(g)^*\omega=\omega.
\end{equation*}
Let $\gg$ be the Lie algebra of $G$ and $\gg^*$ its dual Banach space. Recall that to every element $\xi \in \gg$ we can associate the vector field $v_\xi$ defined as the infinitesimal generator of $\rho(\exp{t\xi})$. Consider now an smooth map $\mu:X\rightarrow \gg^*$ and, for every $\xi \in \gg$ define the function
\begin{align*}
  \mu_\xi :X&\longrightarrow \RR\\ 
  x &\longmapsto \esc{\mu(x),\xi}, 
  \end{align*}
  where $\esc{-,-}$ denotes the natural pairing between $\gg$ and its dual. We say that $\mu$ is a \emph{moment map} for the action of $G$ on $X$ if for every $\xi\in \gg$, the vector field $v_\xi$ is Hamiltonian with Hamiltonian $\mu_\xi$, that is, if
  \begin{equation*}
    d\mu_\xi=-i_{v_\xi}\omega
  \end{equation*}
  or, equivalently, if
  \begin{equation*}
    \esc{d_x\mu(u),\xi}=\omega(u,v_{\xi}(x)),
  \end{equation*}
  for every $u\in T_xX$, where $d_x\mu:T_xX \rightarrow \gg^*$ is the derivative of $\mu$ at $x$.

  \section{Symplectic reduction}

  We are going now to introduce \emph{symplectic reduction}, for which we will need to assume that we have a symplectic action of a Banach Lie group $G$ on a symplectic manifold $(X,\omega)$ that admits a moment map with the following technical conditions. First, assume that $\mu$ is $G$-\emph{equivariant}, that is, 
  \begin{equation*}
