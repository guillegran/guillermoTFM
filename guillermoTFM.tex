...
Let $X$ be a compact Riemann surface of genus $g\geq 2$, $E$ a complex vector bundle on $X$ of rank $n$ and degree $d$ and $h$ a Hermitian metric on $E$. Consider the set $\AA_h$ of all $h$-unitary connections on $E$. Any two $h$-unitary connection can be seen to differ in a $1$-form with values in the bundle $\End(E,h)$, which is just the ``Lie algebra'' of the bundle $\Aut(E,h)$ of $h$-preserving automorphisms of $E$. Therefore, $\AA_h$ is an affine space modeled on $\Omega^1(X,\End(E,h))$. This space admits a non-degenerate skew-symmetric form
\begin{equation*}
  \omega(A,B)=-\int_X \tr(A\wedge B),
\end{equation*}
endowing $\AA_h$, at least formally, with the structure of a symplectic manifold. Strictly speaking, we would need to give in $\AA_h$ the structure of a Banach manifold. This is a technical procedure that we will not detail here, but the essential idea is to give completions of this space with respect to \emph{Sobolev norms}. In a similar fashion, for our purposes we also need to give Sobolev completions of the group $\GG=\Gamma(\Aut(E,h))$, in order to get a Banach Lie group. Go to \cite{atiyahbott,kobayashi} for explicit constructions.

Remember that the Lie group $\GG$ acted on $\AA_h$ by congugation $\nabla\mapsto g\nabla g^{-1}$. We will now show that this action does indeed admit a momentum map, given by
\begin{equation*}
  \mu(\nabla)=-F-2\pi i \mu(E) \id_E \omega_X,
\end{equation*}
where $F$ is the curvature of $\nabla$. In fact, $\mu(\nabla)=-F$ could be also a suitable momentum map, however, the second term is added in there in order to get a non-empty symplectic quotient. Indeed, $\mu^{-1}(0)$ would be empty unless $\deg E=0$. This definition makes sense if we identify the Lie algebra of $\GG$ as $\Lie\ \GG= \Gamma(\End(E,h))$ and its dual with $(\Lie\ \GG)^*=\Omega^2(X,\End(E,h))$, via the pairing
\begin{equation*}
  \esc{\alpha,\xi}=\int_X \tr(\xi \alpha),
\end{equation*}
for $\xi \in \Gamma(\End(E,h))$ and $\alpha \in \Omega^2(X,\End(E,h))$.
Now, to see how $\vec{\xi}$ looks like for an element $\xi \in \Gamma(\End(E,h))$, just compute
\begin{equation*}
  \vec{\xi}(\nabla)=\left.\frac{d}{dt}\right|_{t=0} \exp(t\xi)\nabla \exp(-t\xi)= -\nabla \xi.
\end{equation*}
Let us compute also $d_\nabla \mu (A)$ for some $A \in \Omega^1(X,\End(E,h))$,
\begin{align*}
